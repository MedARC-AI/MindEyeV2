...
backbone_ = model.backbone(b)
print(backbone_.shape)
...
for epoch in progress_bar:
    ...
    for train_i in range(num_iterations_per_epoch):
        with torch.cuda.amp.autocast(dtype=data_type):
            ...
            #     loss_blurry = l1(image_enc_pred, image_enc) * 0
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            for test_i, (behav, past_behav, future_behav, old_behav) in enumerate(test_dl):  
                ...
                for rep in range(3):
                    ...
                    backbone0 = model.backbone(voxel_ridge)
                    if rep==0:
...
print("\n===Finished!===\n")
accelerator.save_state('/weka/proj-fmri/mihirneal/MindEyeV2/src/ablation_ckpt/re_only_prior')
save_ckpt(f're_only_prior')
if ckpt_saving:
    save_ckpt(f're_only_prior')
