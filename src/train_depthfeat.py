...
def unpack_pairs(paired, index):
    upscaled_output = paired[index][0]
    feature_output = paired[index][1]
    return upscaled_output, feature_output

def get_dv2_embeds(dinov2_model, batch):
    with torch.inference_mode():
        res, size = dv2.encode(batch, img_metas=None, rescale=True)
    return res
...
dv2 = torch.hub.load("facebookresearch/dinov2", "dinov2_vitg14_dd").to(device)
dv2.eval()
...
if use_cont:
    ...
    train_augs_512 = AugmentationSequential(
    ...
    train_augs_224 = AugmentationSequential(
        # kornia.augmentation.RandomCrop((480, 480), p=0.3),
        # kornia.augmentation.Resize((512, 512)),
        kornia.augmentation.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.2, hue=0.1, p=0.8),
        kornia.augmentation.RandomGrayscale(p=0.2),
...
if voxel_dims == 1: # 1D data
    voxel2sd = DV2MLP(use_cont=use_cont, in_dim=in_dims[subj_id])
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image, _) in enumerate(train_dl):
        ...
        image_224 = F.interpolate(image, (224, 224), mode='bilinear', align_corners=False, antialias=True)
        ...
        with torch.cuda.amp.autocast(enabled=use_mp):
            autoenc_image = kornia.filters.median_blur(image_224, (15, 15)) if use_blurred_training else image_224
            embeds = get_dv2_embeds(dv2, autoenc_image)
            image_enc, feat = unpack_pairs(embeds, 0)
            image_enc = image_enc * 0.18215
            feat = feat * 0.18215
            if use_cont:
                image_enc_pred, feat_pred, transformer_feats = voxel2sd(voxel, return_transformer_feats=True)
            else:
                image_enc_pred, feat_pred = voxel2sd(voxel)
            ...
            if use_cont:
                ...
                image_aug = (train_augs_512(image_512) - mean)/std
            ...
            loss_1 = F.l1_loss(image_enc_pred, image_enc)
            loss_2 = F.l1_loss(feat_pred, feat)
            mse_loss = loss_1 + loss_2
            del image_512, voxel
    ...
    if local_rank==0: 
        ...
        for val_i, (voxel, image, _) in enumerate(val_dl): 
            with torch.inference_mode():
                ...
                image_d = F.interpolate(image, (224, 224), mode='bilinear', align_corners=False, antialias=True)              
                voxel = voxel.to(device).float()
                voxel = voxel.mean(1)
                
                with torch.cuda.amp.autocast(enabled=use_mp):
                    embeds = get_dv2_embeds(dv2, image_d)
                    image_enc, feat = unpack_pairs(embeds, 0)
                    image_enc = image_enc * 0.18215
                    feat = feat * 0.18215
                    if hasattr(voxel2sd, 'module'):
                        image_enc_pred, feat_pred = voxel2sd.module(voxel)
                    else:
                        image_enc_pred, feat_pred, *_ = voxel2sd(voxel)

                    # mse_loss = F.mse_loss(image_enc_pred, image_enc)
                    loss_1 = F.mse_loss(image_enc_pred, image_enc)
                    loss_2 = F.mse_loss(feat_pred, feat)
                    mse_loss = loss_1 + loss_2
