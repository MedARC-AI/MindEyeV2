...
model.backbone = BrainNetwork(text=True, pool=False, h=hidden_dim, in_dim=2048, seq_len=seq_len, clip_size=2048, out_dim=2048*77).cuda()
...
if lr_scheduler_type == 'linear':
    lr_scheduler = torch.optim.lr_scheduler.LinearLR(
        optimizer,
        total_iters=int(np.floor(num_epochs*(num_train/num_devices/batch_size/4))),
        last_epoch=-1
    )
...
for epoch in progress_bar:
    ...
    loss_img_mse_sum = 0.
    val_loss_img_mse_sum = 0.
    loss_txt_mse_sum = 0.
    val_loss_txt_mse_sum = 0.
    ...
    for train_i, (behav, past_behav, future_behav, old_behav) in enumerate(train_dl):
        with torch.cuda.amp.autocast(dtype=data_type):
            ...
            clip_txt_voxels, clip_voxels_proj = model.backbone(voxel_ridge)

            clip_voxels_norm = nn.functional.normalize(clip_voxels_proj.flatten(1), dim=-1)
            clip_target_norm = nn.functional.normalize(clip_target.flatten(1), dim=-1)

            clip_txt_voxels_norm = nn.functional.normalize(clip_txt_voxels.flatten(1), dim=-1)
            clip_txt_target_norm = nn.functional.normalize(txt_emb.flatten(1), dim=-1)

            mse_img = F.mse_loss(clip_voxels_proj, clip_target)
            mse_txt = F.mse_loss(clip_txt_voxels, txt_emb)
            ...
            if epoch < int(mixup_pct * num_epochs):
                ...
                loss_txt_nce = utils.mixco_nce(
                    clip_txt_voxels_norm,
                    clip_txt_target_norm,
                    temp=.006, 
                    perm=perm, betas=betas, select=select)
            else:
                ...
                loss_txt_nce = utils.soft_clip_loss(
                    clip_txt_voxels_norm,
                    clip_txt_target_norm,
                    temp=epoch_temp)
        
            loss_nce_sum += loss_nce.item()
            loss_txt_nce_sum += loss_txt_nce.item()

            loss_img_mse_sum += mse_img.item()
            loss_txt_mse_sum += mse_txt.item()

            loss = (loss_nce + loss_txt_nce) + (mse_img + mse_txt)
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            for test_i, (behav, past_behav, future_behav, old_behav) in enumerate(test_dl):  
                ...
                if epoch < int(mixup_pct * num_epochs):
                    ...
                    val_loss_txt_nce = utils.mixco_nce(
                        clip_txt_voxels_norm,
                        clip_txt_target_norm,
                        temp=.006, 
                        perm=perm, betas=betas, select=select)
                    ...
                    val_loss_txt_nce = utils.soft_clip_loss(
                        clip_txt_voxels_norm,
                        clip_txt_target_norm,
                        temp=epoch_temp)
        
                val_loss_nce_sum += val_loss_nce.item()
                val_loss_txt_nce_sum += val_loss_txt_nce.item()

                val_loss_img_mse_sum += mse_img.item()
                val_loss_txt_mse_sum += mse_txt.item()

                val_loss = (val_loss_nce + val_loss_txt_nce) + (mse_img + mse_txt)
            ...
            logs = {
                ...
                "train/loss_txt_nce": loss_txt_nce_sum / (train_i + 1),
                "train/loss_img_mse": loss_img_mse_sum / (train_i + 1),
                "train/loss_txt_mse": loss_txt_mse_sum / (train_i + 1),
                "train/loss": losses[train_i],

                "test/val_loss_nce": val_loss_nce_sum / (test_i + 1),
                "test/val_loss_txt_nce": val_loss_txt_nce_sum / (test_i + 1),
                "test/val_loss_img_mse": val_loss_img_mse_sum / (test_i + 1),
                "test/val_loss_txt_mse": val_loss_txt_mse_sum / (test_i + 1),
                "test/val_loss": test_losses[test_i],
                }
            ...
            if plot_umap:
                    ...
                    aligned = clip_txt_voxels[[0]].squeeze(0).detach().cpu().numpy()
                    ...
                    plt.legend()
                    plt.title(f'UMAP projection of two embeddings. Avg dis: {average_distance}. Epoch: {epoch}')
...
if accelerator.is_main_process:
    model = accelerator.unwrap_model(model)
    torch.save(model.state_dict(), "/fsx/proj-fmri/shared/models/fmri-txt-img/mse_proj_drop5.pth")
