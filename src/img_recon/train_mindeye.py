...
if utils.is_interactive():
    ...
    jupyter_args = "--data_path=/fsx/proj-medarc/fmri/natural-scenes-dataset \
                    --model_name=test \
                    --subj=1 --hidden --clip_variant=ViT-L/14 --batch_size=8 --n_samples_save=0 \
...
train_url = [f"{data_path}/webdataset_avg_split/train/train_subj0{subj}_{i}.tar" for i in range(18)]
val_url = [f"{data_path}/webdataset_avg_split/val/val_subj0{subj}_{i}.tar" for i in range(1)]
print(train_url,"\n",val_url)
...
if voxel2clip_path!="None":
    checkpoint = torch.load(voxel2clip_path, map_location='cuda')
...
if n_samples_save > 0 and hidden:
    ...
    vd_cache_dir = '/fsx/proj-fmri/shared/cache/mmodels--shi-labs--versatile-diffusions/snapshots/2926f8e11ea526b562cd592b099fcf9c2985d0b7'
    ...
    sd_cache_dir = '/fsx/proj-fmri/shared/cache/models--lambdalabs--sd-image-variations-diffusers/snapshots/a2a13984e57db80adcc9e3f85d568dcccb9b29fc'
...
# params for wandb
if wandb_log: # only use main process for wandb logging
    import wandb
    wandb_project = 'mi2_diffPrior'
    wandb_run = model_name
    wandb_notes = 'Setting diffusion prior to work with pytorch lightning'
...
trainer = pl.Trainer(limit_train_batches=batch_size, max_epochs=2, default_root_dir="../logs/tmp/ckpt/")
trainer.fit(model=diffusion_prior, train_dataloaders=train_dl, val_dataloaders=val_dl)
