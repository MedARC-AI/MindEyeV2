...
class BrainDiffusionPrior(DiffusionPrior, pl.LightningModule):
    """ 
    ...
    """
    def __init__(self, num_devices, subj, out_dim, clip_size, use_projector, *args, **kwargs):
    ...
    def init_voxel2clip(self):
        ...
        if self.subj == 1:
            ...
            num_voxels = 14386
        voxel2clip_kwargs = dict(in_dim=num_voxels,out_dim=self.out_dim,clip_size=self.clip_size,use_projector=self.use_projector)
    ...
    def configure_optimizers(self, batch_size=8, lr_scheduler_type='cycle'):
        no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']
        opt_grouped_parameters = [
            {'params': [p for n, p in self.net.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 1e-2},
            {'params': [p for n, p in self.net.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0},
        ...
        num_train = 8559 + 300
        optimizer = torch.optim.AdamW(opt_grouped_parameters, lr=3e-4) #lr = self.max_lr
        lr_scheduler = {
            'linear': torch.optim.lr_scheduler.LinearLR(
                optimizer,
                total_iters=int(self.trainer.max_epochs*(num_train//(batch_size*self.num_devices))),
                last_epoch=-1
            ),
            'cycle': torch.optim.lr_scheduler.OneCycleLR(
                ...
                # max_lr=self.max_lr,
                max_lr = 3e-4,
                total_steps=int(self.trainer.max_epochs*(num_train//(batch_size*self.num_devices))),
                final_div_factor=1000,
                last_epoch=-1, pct_start=(2/self.trainer.max_epochs)
            )
        }
        return [optimizer], [lr_scheduler[lr_scheduler_type]]
    
    def training_step(self, batch, batch_idx, mixup_pct=0.33, num_epochs=240, clip_size=768):
        loss_nce_sum = 0.
        loss_prior_sum = 0.
        sims_base = 0.
        ...
        voxel, image, coco = batch
        image = self.image_aug(image)
        soft_loss_temps = utils.cosine_anneal(0.004, 0.0075, num_epochs - int(mixup_pct * num_epochs))
        if epoch < int(mixup_pct * num_epochs):
            voxel, perm, betas, select = utils.mixco(voxel)

        image_embed = self.clip.embed_image(image).float()
        clip_voxels, clip_voxels_proj = self.voxel2clip(voxel)
        clip_voxels = clip_voxels.view(len(voxel),-1,clip_size)

        loss_prior, _ = self.forward(text_embed=clip_voxels, image_embed=image_embed)
        ...
        labels = torch.arange(len(clip_target_norm))
        ...
        self.log("train/cosine_sim_base", sims_base)
        self.log("train/fwd_pct_correct", fwd_percent_correct)
        self.log("train/bwd_pct_correct", bwd_percent_correct)
        self.log("train/loss_nce", loss_nce_sum)
        self.log("train/loss_prior", loss_prior_sum)
        return loss
    
    def validation_step(self, batch, batch_idx, mixup_pct=0.33, num_epochs=240, clip_size=768):
        val_loss_nce_sum = 0.
        ...
        val_loss_prior, _ = self.forward(text_embed=clip_voxels, image_embed=image_embed)
        clip_voxels_norm = nn.functional.normalize(clip_voxels_proj.flatten(1), dim=-1)
        clip_target_norm = nn.functional.normalize(image_embed.flatten(1), dim=-1)
        ...
        self.log("val/cosine_sim_base", val_sims_base)
        self.log("val/val_fwd_pct_correct", val_fwd_percent_correct)
        self.log("val/val_bwd_pct_correct", val_bwd_percent_correct)
        self.log("val/loss_nce", val_loss_nce_sum)
        self.log("val/loss_prior", val_loss_prior_sum)
        return val_loss
...
class Clipper(torch.nn.Module):
    def __init__(self, clip_variant, clamp_embs=False, norm_embs=False,
                 hidden_state=False, device=torch.device('cuda')):
        super().__init__()
...
class OpenClipper(torch.nn.Module):
    def __init__(self, clip_variant='ViT-H-14', hidden_state=False, norm_embs=False, device=torch.device('cuda')):
        super().__init__()
        print(clip_variant, device)
