...
for subj in [1]:
    for mode in ["imagery", "vision"]:
        for model_name in [f"pretrained_subj{subj:02d}_40sess_ext1_hypatia_v2", f"pretrained_subj{subj:02d}_40sess_hypatia_vd2"]:#f"pretrained_subj{subj:02d}irf_40sess_hypatia_imageryrf_all_no_blurry", f"pretrained_subj{subj:02d}irf_40sess_hypatia_imageryrf_vision_no_blurry"]:
            try:
                all_recons = torch.load(f"evals/{model_name}/{model_name}_all_enhancedrecons_{mode}.pt")
            except:
                all_recons = torch.load(f"evals/{model_name}/{model_name}_all_recons_{mode}.pt")
