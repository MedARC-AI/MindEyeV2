...
class BrainNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        return c_i, t, pooled, b, d
        
model.backbone = BrainNetwork(h=hidden_dim, in_dim=2048, seq_len=seq_len, clip_size=2048, out_dim=2048*77).cuda()
...
clipimg_, clip_, pooled_, blur_, depth_ = model.backbone(b)
print(clipimg_.shape, clip_.shape, pooled_.shape)
...
model.backbone = BrainNetwork(h=hidden_dim, in_dim=2048, seq_len=seq_len, clip_size=2048, out_dim=2048*77) 
utils.count_params(model.backbone)
utils.count_params(model)
utils.count_params(model)
...
for epoch in progress_bar:
    ...
    for train_i, (behav, past_behav, future_behav, old_behav) in enumerate(train_dl):
        with torch.cuda.amp.autocast(dtype=data_type):
            ...
            clip_voxels, txt_voxels, pooled_voxels, blurry_image_enc_, depth_image_enc_ = model.backbone(voxel_ridge)
            ...
            pooled_voxel_norm = nn.functional.normalize(pooled_voxels.flatten(1), dim=-1)
            pooled_emb_norm = nn.functional.normalize(pooled_emb.flatten(1), dim=-1)
            ...
            if epoch < int(mixup_pct * num_epochs):                
                ...
                loss_pooled = utils.soft_clip_loss(
                    pooled_voxel_norm,
                    pooled_emb_norm,
                    temp=epoch_temp)

            mse_img = F.mse_loss(clip_voxels, clip_target)
            mse_txt = F.mse_loss(txt_voxels, txt_emb)
            mse_pooled = F.mse_loss(pooled_voxels, pooled_emb)

            loss_mse_img += mse_img.item()
            loss_mse_txt += mse_txt.item()
            loss_mse_pooled += mse_pooled.item()
        
            loss_clip_total += loss_clip.item()
            loss_clip *= clip_scale
            loss_txt_total += loss_txt.item()
            loss_txt *= clip_scale
            loss_pooled_total += loss_pooled.item()
            loss_pooled *= clip_scale

            mse_loss = mse_img + mse_txt + mse_pooled
            loss = loss_clip + loss_pooled + loss_txt + mse_loss
    ...
    if local_rank==0:
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            for test_i, (behav, past_behav, future_behav, old_behav) in enumerate(test_dl):  
                ...
                txt_emb, pooled_emb = text_embeds(pipe, cap)
                voxel_ridge = model.ridge(voxel).unsqueeze(1)
                ...
                loss_pooled = utils.soft_clip_loss(
                    ...
                    pooled_emb_norm,
                    temp=.006)
                test_loss_pooled_total += loss_pooled.item()
                loss_pooled = loss_pooled * clip_scale

                mse_img = F.mse_loss(clip_voxels, clip_target)
                mse_txt = F.mse_loss(txt_voxels, txt_emb)
                mse_pooled = F.mse_loss(pooled_voxels, pooled_emb)

                test_loss_mse_img += mse_img.item()
                test_loss_mse_txt += mse_txt.item()
                test_loss_mse_pooled += mse_pooled.item()
                
                test_mse_loss = mse_img + mse_txt + mse_pooled
                loss = loss_clip + loss_pooled + loss_text + test_mse_loss
            ...
            logs = {"train/loss": np.mean(losses[-(train_i+1):]),
                ...
                "train/loss_txt_total": loss_txt_total / (train_i + 1),
                "train/loss_pooled_total": loss_pooled_total / (train_i + 1),
                "train/loss_mse_img": loss_mse_img / (train_i + 1),
                "train/loss_mse_txt": loss_mse_txt / (train_i + 1),
                "train/loss_mse_pooled": loss_mse_pooled / (train_i + 1),
                "test/test_loss_clip_total": test_loss_clip_total / (test_i + 1),
                "test/test_loss_txt_total": test_loss_txt_total / (test_i + 1),
                "test/test_loss_pooled_total": test_loss_pooled_total / (test_i + 1),
                "test/test_loss_mse_img": test_loss_mse_img / (test_i + 1),
                "test/test_loss_mse_txt": test_loss_mse_txt / (test_i + 1),
                "test/test_loss_mse_pooled": test_loss_mse_pooled / (test_i + 1),
...
if accelerator.is_main_process:
    ...
    torch.save(model.state_dict(), "/fsx/proj-fmri/shared/models/fmri-txt-img/new/model_trained_mse.pth")
