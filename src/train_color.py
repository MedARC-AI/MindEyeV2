...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image, _) in enumerate(train_dl):
        ...
        image_color = F.interpolate(image, (8, 8), mode='bilinear', align_corners=False, antialias=True)
        image_512 = nn.functional.interpolate(image_color, size=(512, 512), mode='nearest')
    ...
    if local_rank==0: 
        ...
        for val_i, (voxel, image, _) in enumerate(val_dl): 
            with torch.inference_mode():
                image = image.to(device).float()
                image_color = F.interpolate(image, (8, 8), mode='bilinear', align_corners=False, antialias=True)
                image = nn.functional.interpolate(image_color, size=(512, 512), mode='nearest')
