...
import utils_mv1 as utils
from models_mev1 import Clipper, BrainNetwork, BrainDiffusionPrior, BrainDiffusionPriorOld, VersatileDiffusionPriorNetwork
...
if True:
    # Example use
    jupyter_args = "--data_path=/weka/proj-fmri/shared/mindeyev1/naturalscenesdataset \
                    --model_name=MEV1 \
                    --subj=1 --hidden --clip_variant=ViT-L/14 --batch_size=32 --n_samples_save=0 \
                    --max_lr=3e-4 --mixup_pct=.33 --num_epochs=240 --ckpt_interval=5 --use_image_aug --no-wandb_log"
...
parser.add_argument(
    "--plot_umap",action=argparse.BooleanOptionalAction,default=True,
...
parser.add_argument(
    "--vd_cache_dir", type=str, default='/weka/proj-fmri/cache/models--shi-labs--versatile-diffusion/snapshots/2926f8e11ea526b562cd592b099fcf9c2985d0b7',
...
train_url = "{" + f"{data_path}/webdataset_avg_new/train/train_subj0{subj}_" + "{0..17}.tar," + f"{data_path}/webdataset_avg_new/val/val_subj0{subj}_0.tar" + "}"
val_url = f"{data_path}/webdataset_avg_new/test/test_subj0{subj}_" + "{0..1}.tar"
print(train_url,"\n",val_url)
meta_url = f"{data_path}/webdataset_avg_new/metadata_subj0{subj}.json"
num_train = 8559 + 300
num_val = 982
...
test_vox_indices = []
test_73k_images = []
for test_i, (behav, past_behav, future_behav) in enumerate(val_dl):
...
train_vox_indices = []
train_73k_images = []
for train_i, (behav, past_behav, future_behav) in enumerate(train_dl):
...
if n_samples_save > 0 and hidden:
    ...
    sd_cache_dir = '/weka/proj-fmri/shared/cache/models--lambdalabs--sd-image-variations-diffusers/snapshots/a2a13984e57db80adcc9e3f85d568dcccb9b29fc'
