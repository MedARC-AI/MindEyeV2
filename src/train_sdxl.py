...
if use_cont:
    mixup_pct = -1
    if cont_model == 'cnx':
        cnx = ConvnextXL('/fsx/proj-fmri/shared/models/convnextv2/convnext_xlarge_alpha0.75_fullckpt.pth').to(torch.float16)
...
outdir = f'/fsx/proj-fmri/shared/models/vae_sdxl'
...
if accelerator.is_main_process: print('Creating voxel2sd...')

in_dims = {'01': 15724, '02': 14278, '05': 13039, '07':12682}
if voxel_dims == 1: # 1D data
    voxel2sd = Voxel2StableDiffusionXL(use_cont=use_cont, in_dim=in_dims[subj_id], ups_mode=ups_mode)
...
try:
    ...
    if accelerator.is_main_process: print('Cannot count params for voxel2sd (probably because it has Lazy layers)')


if accelerator.is_main_process: print('Pulling NSD webdataset data...')
...
if accelerator.is_main_process: print('Prepping train and validation dataloaders...')
num_train = 8559 + 300
num_val = 982
...
voxel2sd, optimizer, train_dl, lr_scheduler = accelerator.prepare(voxel2sd, optimizer, train_dl, lr_scheduler)
...
if resume_from_ckpt:
    ckpt_path = os.path.join(outdir, "last")
    print("\n---resuming from ckpt_path---\n", ckpt_path)
    accelerator.load_state("/fsx/proj-fmri/shared/models/vae_sdxl/last/")
...
if ckpt_path is not None:
    ...
    # # sch_dict['total_steps'] = int(sch_dict['total_steps']) + epoch * (num_train//global_batch_size)
...
progress_bar = tqdm(range(epoch, num_epochs), ncols=150, disable=(not accelerator.is_local_main_process))
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image, _) in enumerate(train_dl):
        ...
        image_1024 = F.interpolate(image, (1024, 1024), mode='bilinear', align_corners=False, antialias=True)
        voxel = voxel.to(device).to(torch.float16)
        ...
        with torch.cuda.amp.autocast(enabled=use_mp):
            autoenc_image = kornia.filters.median_blur(image_1024, (15, 15)) if use_blurred_training else image_1024
            image_enc = autoenc.encode(2*autoenc_image-1).latent_dist.sample() * 0.18215
            if use_cont:
            ...
            if use_cont:
                ...
                image_norm = image_norm.to(torch.float16)
                image_aug = (train_augs(image_512) - mean)/std
                image_aug = image_aug.to(torch.float16)
            ...
            del image_512, image_1024, voxel
    ...
    if accelerator.is_main_process:
        ...
        for val_i, (voxel, image, _) in enumerate(val_dl): 
            with torch.inference_mode():
                image = image.to(device).to(torch.float16)
                image = F.interpolate(image, (1024, 1024), mode='bilinear', align_corners=False, antialias=True)              
                voxel = voxel.to(device).to(torch.float16)
                voxel = voxel.mean(1)
                
                with torch.cuda.amp.autocast(enabled=use_mp):
                    autoenc = autoenc.to(torch.float16)
                    image_enc = autoenc.encode(2*image-1).latent_dist.sample() * 0.18215
                    if hasattr(voxel2sd, 'module'):
        ...
        if (not save_at_end and ckpt_saving) or (save_at_end and epoch == num_epochs - 1):
            ...
            if val_loss < best_val_loss:
                ...
                ckpt_path = os.path.join(outdir, "best")
                accelerator.save_state(ckpt_path)
            else:
                print(f'not best - val_loss: {val_loss:.3f}, best_val_loss: {best_val_loss:.3f}')
            if val_ssim > best_ssim:
                best_ssim = val_ssim
                ckpt_path = os.path.join(outdir, "ssim")
                accelerator.save_state(ckpt_path)
            else:
            ...
            if (ckpt_interval is not None and (epoch + 1) % ckpt_interval == 0) or epoch == num_epochs - 1:
                ckpt_path = os.path.join(outdir, f"checkpoint-epoch{(epoch+1):03d}'")
                accelerator.save_state(ckpt_path)
            try:
                orig = image
                if reconst is None:
                    autoenc = autoenc.to(torch.float16)
                    image_enc_pred = image_enc_pred.to(torch.float16)
        ...
        if accelerator.is_main_process: print(logs)
