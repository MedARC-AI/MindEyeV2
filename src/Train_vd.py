        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            ...
            if (epoch == num_epochs-1) or (epoch % ckpt_interval == 0):
                ...
                if use_prior and visualize_prior: # output recons every ckpt
                    ...
                    grid1, _, _, _ = utils.reconstruction(
                        image=image[:1], 
                        voxel=voxel[:1],
                        clip_extractor=clip_extractor, 
                        diff_embed=prior_out[:1],
                        unet=unet, vae=vae,
                    ...
                    grid2, _, _, _ = utils.reconstruction(
                        image=image[1:2], 
                        voxel=voxel[1:2],
                        clip_extractor=clip_extractor, 
                        diff_embed=prior_out[1:2],
                        unet=unet, vae=vae,
                        noise_scheduler=noise_scheduler,
                        diffusion_priors = model.diffusion_prior,
                        num_inference_steps = num_inference_steps,
                    ...
                    grid3, _, _, _ = utils.reconstruction(
                        image=image[2:3], 
                        voxel=voxel[2:3],
                        clip_extractor=clip_extractor, 
                        diff_embed=prior_out[2:3],
                        unet=unet, vae=vae,
                        noise_scheduler=noise_scheduler,
                        diffusion_priors = model.diffusion_prior,
                        num_inference_steps = num_inference_steps,
                    ...
                    grid4, _, _, _ = utils.reconstruction(
                        image=image[3:4], 
                        voxel=voxel[3:4],
                        clip_extractor=clip_extractor, 
                        diff_embed=prior_out[3:4],
                        unet=unet, vae=vae,
                        noise_scheduler=noise_scheduler,
                        diffusion_priors = model.diffusion_prior,
                        num_inference_steps = num_inference_steps,
                    ...
                    grid5, _, _, _ = utils.reconstruction(
                        image=image[4:5], 
                        voxel=voxel[4:5],
                        clip_extractor=clip_extractor, 
                        diff_embed=prior_out[4:5],
                        unet=unet, vae=vae,
                        noise_scheduler=noise_scheduler,
                        diffusion_priors = model.diffusion_prior,
                        num_inference_steps = num_inference_steps,
                    ...
                    if wandb_log:
                        ...
                        logs[f"val/recons1"] = wandb.Image(grid1, caption=f"epoch{epoch:03d}")
                        plt.close()
                        logs[f"val/recons2"] = wandb.Image(grid2, caption=f"epoch{epoch:03d}")
                        plt.close()
                        logs[f"val/recons3"] = wandb.Image(grid3, caption=f"epoch{epoch:03d}")
                        plt.close()
                        logs[f"val/recons4"] = wandb.Image(grid4, caption=f"epoch{epoch:03d}")
                        plt.close()
                        logs[f"val/recons5"] = wandb.Image(grid5, caption=f"epoch{epoch:03d}")
                        plt.close()
