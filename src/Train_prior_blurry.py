...
backbone_,  blur_ = model.backbone(b)
print(backbone_.shape, blur_[0].shape, blur_[1].shape)
...
for epoch in progress_bar:
    ...
    for train_i in range(num_iterations_per_epoch):
        with torch.cuda.amp.autocast(dtype=data_type):
            ...
            backbone, blurry_image_enc_ = model.backbone(voxel_ridge)
            ...
            #             clip_target_norm,
            #             temp=.006,
            #             perm=perm, betas=betas, select=select) * 0
            #     else:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            for test_i, (behav, past_behav, future_behav, old_behav) in enumerate(test_dl):  
                ...
                for rep in range(3):
                    ...
                    backbone0, blurry_image_enc_ = model.backbone(voxel_ridge)
                    if rep==0:
...
print("\n===Finished!===\n")
accelerator.save_state('/weka/proj-fmri/mihirneal/MindEyeV2/src/ablation_ckpt/re_prior_blurry')
save_ckpt(f're_prior_blurry')
if ckpt_saving:
    save_ckpt(f're_prior_blurry')
